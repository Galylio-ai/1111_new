          ...
          <div className="card card-pad">
            ...
            <ul className="mt-1 space-y-1.5">
              {predictions.map((p) => (
                <li
                  key={p.name}
                  className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-2 rounded-xl border border-white/5 bg-bg-800/40 px-2 py-2.5 text-sm transition hover:border-white/15"
                >
                  <span className="truncate font-semibold text-white">{p.name}</span>
                  <span className="flex w-20 justify-center">
                    <span className={`rounded-md px-2 py-0.5 text-[11px] font-bold ${p.reco === "Acheter" ? "bg-emerald-500/15 text-emerald-300" : "bg-amber-500/15 text-amber-300"}`}>
                      {p.reco}
                    </span>
                  </span>
                  <span className={`flex w-20 items-center justify-end gap-0.5 font-bold tabular-nums ${p.up ? "text-red-300" : "text-emerald-300"}`}>
                  ...
                  <span className="hidden w-24 items-center justify-end gap-2 sm:flex">
                    ...
                    <span className="text-[11px] tabular-nums text-white/60">{p.conf}%</span>
                  </span>
                </li>
              ))}
            </ul>
