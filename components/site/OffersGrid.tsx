      ...
      {items.map((p, i) => {
        ...
        return (
          <a
            ...
            className="reveal-up card group relative overflow-hidden p-3 transition hover:-translate-y-1 hover:border-white/20 hover:shadow-[0_0_30px_-10px_rgba(225,29,45,0.5)]"
          >
            <span className="absolute right-3 top-3 z-10 rounded-md bg-brand-red px-2 py-0.5 text-[11px] font-bold text-white shadow">
              {pct}%
            </span>
            <div className="relative h-36 w-full overflow-hidden rounded-xl bg-white sm:h-40 md:h-44">
              <Image
                src={p.image}
                alt={p.name}
                fill
            ...
            <div className="mt-1 flex items-baseline gap-2">
              ...
              <span className="text-xs text-white/40 line-through">{p.oldPrice.toLocaleString("fr-FR")} DT</span>
            </div>
