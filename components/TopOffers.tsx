  ...
  return (
    <section className="mx-auto mt-6 max-w-[1600px] px-3 sm:px-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link className="text-xs font-medium text-slate-500 transition hover:text-brand-gold dark:text-white/70" href="/promotions">
      ...
      <div className="relative">
        <button
          onClick={() => scrollBy(-1)}
          className="absolute -left-3 top-1/2 z-10 hidden -translate-y-1/2 rounded-full border border-bg-border bg-bg-card p-2 text-slate-700 shadow-card hover:bg-bg-700 dark:text-white md:flex"
        ...
        <button
          onClick={() => scrollBy(1)}
          className="absolute -right-3 top-1/2 z-10 hidden -translate-y-1/2 rounded-full border border-bg-border bg-bg-card p-2 text-slate-700 shadow-card hover:bg-bg-700 dark:text-white md:flex"
          aria-label="Suivant"
        >
        ...
        >
          {topOffers.map((p) => (
            <Link
              key={p.id}
              href={`/produit/${p.id}`}
              className="card group relative w-[200px] sm:w-[240px] md:w-[260px] shrink-0 snap-start overflow-hidden p-3 transition hover:-translate-y-0.5 hover:border-slate-300 dark:hover:border-white/20"
            >
              <span className="absolute right-3 top-3 z-10 rounded-md bg-brand-red px-2 py-0.5 text-[11px] font-bold text-white shadow">
                {p.discount} DT
              </span>
              <div className="relative h-36 sm:h-40 md:h-44 w-full overflow-hidden rounded-xl bg-gradient-to-br from-slate-100 to-white ring-1 ring-inset ring-slate-200 dark:from-white/[0.06] dark:to-white/[0.01] dark:ring-white/5">
                <Image
                  src={p.image}
                  alt={p.name}
                  fill
              ...
              <div className="mt-1 flex items-baseline gap-2">
                ...
                <span className="text-xs text-slate-400 line-through dark:text-white/40">
