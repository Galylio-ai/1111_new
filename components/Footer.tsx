      ...
      <div className="card card-pad">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-[1.2fr_2fr_1fr]">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {columns.map((c) => (
              <div key={c.title}>
                <div className="mb-2 text-sm font-semibold text-white">{c.title}</div>
          ...
          <div>
            <div className="text-sm font-semibold text-white">Suivez-nous</div>
            <div className="mt-2 flex items-center gap-2">
              {[Facebook, Instagram, Youtube, Music2].map((I, idx) => (
                <a
                  key={idx}
                  href="#"
                  className="rounded-lg border border-bg-border bg-bg-800/60 p-2 text-white/80 hover:border-white/30 hover:text-white"
                >
                  <I className="h-4 w-4" />
                </a>
              ))}
            ...
            <div className="mt-4 flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3">
              <div className="text-xs">
                <div className="font-semibold text-emerald-300">Excellent</div>
              ...
              <div className="text-lg font-extrabold text-white">4.8/5</div>
            </div>
          </div>
        </div>

        <div className="mt-5 flex flex-col items-start justify-between gap-2 border-t border-bg-border pt-4 text-[11px] text-white/50 md:flex-row md:items-center">
